...
//	Last Modified (Original)	: November 1998
//
...
const struct cipher_descriptor aes_desc =
{
    "aes",
    1,
    16, 32, 16, 10,
...
#pragma mark - INLINE
static inline unsigned _setup_mix(unsigned temp)
{
    return (Te4_3[NBS_BYTE(temp, 2)]) ^ (Te4_2[NBS_BYTE(temp, 1)]) ^ (Te4_1[NBS_BYTE(temp, 0)]) ^ (Te4_0[NBS_BYTE(temp, 3)]);
}
...
int aes_setup(const unsigned char *key, int keylen, int num_rounds, cipher_state *cs)
{
	...
	for (;;) {
	    temp  = rk[3];
	    rk[4] = rk[0] ^ _setup_mix(temp) ^ rcon[i];
	    rk[5] = rk[1] ^ rk[4];
	    rk[6] = rk[2] ^ rk[5];
	    rk[7] = rk[3] ^ rk[6];
	    if (++i == 10) {
	...
	for (;;) {
	    temp = rk[5];
	    rk[ 6] = rk[ 0] ^ _setup_mix(temp) ^ rcon[i];
	    rk[ 7] = rk[ 1] ^ rk[ 6];
	    rk[ 8] = rk[ 2] ^ rk[ 7];
	    rk[ 9] = rk[ 3] ^ rk[ 8];
	    if (++i == 8) {
	...
	for (;;) {
	    temp = rk[7];
	    rk[ 8] = rk[ 0] ^ _setup_mix(temp) ^ rcon[i];
	    rk[ 9] = rk[ 1] ^ rk[ 8];
	    rk[10] = rk[ 2] ^ rk[ 9];
	    rk[11] = rk[ 3] ^ rk[10];
	    if (++i == 7) {
	    ...
	    temp = rk[11];
	    rk[12] = rk[ 4] ^ _setup_mix(RORc(temp, 8));
	    rk[13] = rk[ 5] ^ rk[12];
	    rk[14] = rk[ 6] ^ rk[13];
	    rk[15] = rk[ 7] ^ rk[14];
	    rk += 8;
    ...
    *rk   = *rrk;
    ...
    for (i = 1; i < cs->aes.nr; i++) {
	rrk -= 4;
	rk  += 4;

	temp = rrk[0]; rk[0] = Tks0[NBS_BYTE(temp, 3)] ^ Tks1[NBS_BYTE(temp, 2)] ^ Tks2[NBS_BYTE(temp, 1)] ^ Tks3[NBS_BYTE(temp, 0)];
	temp = rrk[1]; rk[1] = Tks0[NBS_BYTE(temp, 3)] ^ Tks1[NBS_BYTE(temp, 2)] ^ Tks2[NBS_BYTE(temp, 1)] ^ Tks3[NBS_BYTE(temp, 0)];
	temp = rrk[2]; rk[2] = Tks0[NBS_BYTE(temp, 3)] ^ Tks1[NBS_BYTE(temp, 2)] ^ Tks2[NBS_BYTE(temp, 1)] ^ Tks3[NBS_BYTE(temp, 0)];
	temp = rrk[3]; rk[3] = Tks0[NBS_BYTE(temp, 3)] ^ Tks1[NBS_BYTE(temp, 2)] ^ Tks2[NBS_BYTE(temp, 1)] ^ Tks3[NBS_BYTE(temp, 0)];
    }
...
int aes_encrypt(const unsigned char *pt, unsigned char *ct, const cipher_state *cs)
{
    ...
    LOAD32(s0, pt      ); s0 ^= rk[0];
    LOAD32(s1, pt  +  4); s1 ^= rk[1];
    LOAD32(s2, pt  +  8); s2 ^= rk[2];
    LOAD32(s3, pt  + 12); s3 ^= rk[3];

    r = Nr >> 1;
    for (;;) {
	t0=Te0(NBS_BYTE(s0,3)) ^ Te1(NBS_BYTE(s1,2)) ^ Te2(NBS_BYTE(s2,1)) ^ Te3(NBS_BYTE(s3,0)) ^ rk[4];
	t1=Te0(NBS_BYTE(s1,3)) ^ Te1(NBS_BYTE(s2,2)) ^ Te2(NBS_BYTE(s3,1)) ^ Te3(NBS_BYTE(s0,0)) ^ rk[5];
	t2=Te0(NBS_BYTE(s2,3)) ^ Te1(NBS_BYTE(s3,2)) ^ Te2(NBS_BYTE(s0,1)) ^ Te3(NBS_BYTE(s1,0)) ^ rk[6];
	t3=Te0(NBS_BYTE(s3,3)) ^ Te1(NBS_BYTE(s0,2)) ^ Te2(NBS_BYTE(s1,1)) ^ Te3(NBS_BYTE(s2,0)) ^ rk[7];
	...
	s0=Te0(NBS_BYTE(t0,3)) ^ Te1(NBS_BYTE(t1,2)) ^ Te2(NBS_BYTE(t2,1)) ^ Te3(NBS_BYTE(t3,0)) ^ rk[0];
	s1=Te0(NBS_BYTE(t1,3)) ^ Te1(NBS_BYTE(t2,2)) ^ Te2(NBS_BYTE(t3,1)) ^ Te3(NBS_BYTE(t0,0)) ^ rk[1];
	s2=Te0(NBS_BYTE(t2,3)) ^ Te1(NBS_BYTE(t3,2)) ^ Te2(NBS_BYTE(t0,1)) ^ Te3(NBS_BYTE(t1,0)) ^ rk[2];
	s3=Te0(NBS_BYTE(t3,3)) ^ Te1(NBS_BYTE(t0,2)) ^ Te2(NBS_BYTE(t1,1)) ^ Te3(NBS_BYTE(t2,0)) ^ rk[3];
    }

    s0=(Te4_3[NBS_BYTE(t0,3)]) ^ (Te4_2[NBS_BYTE(t1,2)]) ^ (Te4_1[NBS_BYTE(t2,1)]) ^ (Te4_0[NBS_BYTE(t3,0)]) ^ rk[0]; STORE32(s0,ct);
    s1=(Te4_3[NBS_BYTE(t1,3)]) ^ (Te4_2[NBS_BYTE(t2,2)]) ^ (Te4_1[NBS_BYTE(t3,1)]) ^ (Te4_0[NBS_BYTE(t0,0)]) ^ rk[1]; STORE32(s1,ct+ 4);
    s2=(Te4_3[NBS_BYTE(t2,3)]) ^ (Te4_2[NBS_BYTE(t3,2)]) ^ (Te4_1[NBS_BYTE(t0,1)]) ^ (Te4_0[NBS_BYTE(t1,0)]) ^ rk[2]; STORE32(s2,ct+ 8);
    s3=(Te4_3[NBS_BYTE(t3,3)]) ^ (Te4_2[NBS_BYTE(t0,2)]) ^ (Te4_1[NBS_BYTE(t1,1)]) ^ (Te4_0[NBS_BYTE(t2,0)]) ^ rk[3]; STORE32(s3,ct+12);
...
int aes_decrypt(const unsigned char *ct, unsigned char *pt, const cipher_state *cs)
{
    ...
    LOAD32(s0, ct      ); s0 ^= rk[0];
    LOAD32(s1, ct  +  4); s1 ^= rk[1];
    LOAD32(s2, ct  +  8); s2 ^= rk[2];
    LOAD32(s3, ct  + 12); s3 ^= rk[3];

    r = Nr >> 1;
    for (;;) {
	t0=Td0(NBS_BYTE(s0,3)) ^ Td1(NBS_BYTE(s3,2)) ^ Td2(NBS_BYTE(s2,1)) ^ Td3(NBS_BYTE(s1,0)) ^ rk[4];
	t1=Td0(NBS_BYTE(s1,3)) ^ Td1(NBS_BYTE(s0,2)) ^ Td2(NBS_BYTE(s3,1)) ^ Td3(NBS_BYTE(s2,0)) ^ rk[5];
	t2=Td0(NBS_BYTE(s2,3)) ^ Td1(NBS_BYTE(s1,2)) ^ Td2(NBS_BYTE(s0,1)) ^ Td3(NBS_BYTE(s3,0)) ^ rk[6];
	t3=Td0(NBS_BYTE(s3,3)) ^ Td1(NBS_BYTE(s2,2)) ^ Td2(NBS_BYTE(s1,1)) ^ Td3(NBS_BYTE(s0,0)) ^ rk[7];

	rk += 8;
	if(--r==0){break;}

	s0=Td0(NBS_BYTE(t0,3)) ^ Td1(NBS_BYTE(t3,2)) ^ Td2(NBS_BYTE(t2,1)) ^ Td3(NBS_BYTE(t1,0)) ^ rk[0];
	s1=Td0(NBS_BYTE(t1,3)) ^ Td1(NBS_BYTE(t0,2)) ^ Td2(NBS_BYTE(t3,1)) ^ Td3(NBS_BYTE(t2,0)) ^ rk[1];
	s2=Td0(NBS_BYTE(t2,3)) ^ Td1(NBS_BYTE(t1,2)) ^ Td2(NBS_BYTE(t0,1)) ^ Td3(NBS_BYTE(t3,0)) ^ rk[2];
	s3=Td0(NBS_BYTE(t3,3)) ^ Td1(NBS_BYTE(t2,2)) ^ Td2(NBS_BYTE(t1,1)) ^ Td3(NBS_BYTE(t0,0)) ^ rk[3];
    }

    s0=(Td4[NBS_BYTE(t0,3)] & 0xff000000) ^ (Td4[NBS_BYTE(t3,2)] & 0x00ff0000) ^ (Td4[NBS_BYTE(t2,1)] & 0x0000ff00) ^ (Td4[NBS_BYTE(t1,0)] & 0x000000ff) ^ rk[0]; STORE32(s0,pt);
    s1=(Td4[NBS_BYTE(t1,3)] & 0xff000000) ^ (Td4[NBS_BYTE(t0,2)] & 0x00ff0000) ^ (Td4[NBS_BYTE(t3,1)] & 0x0000ff00) ^ (Td4[NBS_BYTE(t2,0)] & 0x000000ff) ^ rk[1]; STORE32(s1,pt+ 4);
    s2=(Td4[NBS_BYTE(t2,3)] & 0xff000000) ^ (Td4[NBS_BYTE(t1,2)] & 0x00ff0000) ^ (Td4[NBS_BYTE(t0,1)] & 0x0000ff00) ^ (Td4[NBS_BYTE(t3,0)] & 0x000000ff) ^ rk[2]; STORE32(s2,pt+ 8);
    s3=(Td4[NBS_BYTE(t3,3)] & 0xff000000) ^ (Td4[NBS_BYTE(t2,2)] & 0x00ff0000) ^ (Td4[NBS_BYTE(t1,1)] & 0x0000ff00) ^ (Td4[NBS_BYTE(t0,0)] & 0x000000ff) ^ rk[3]; STORE32(s3,pt+12);
