//
//	sosemanuk.c
//	Authors / Developers		: Come Berbain, Olivier Billet, Anne Canteaut, Nicolas Courtois,
...
#define T32(x) ((x) & (unsigned)0xFFFFFFFF)
...
#define KA(zc, x0, x1, x2, x3) do {		\
    x0 ^= cs->sosemanuk.kc[(zc)];		\
    x1 ^= cs->sosemanuk.kc[(zc) + 1];		\
    x2 ^= cs->sosemanuk.kc[(zc) + 2];		\
    x3 ^= cs->sosemanuk.kc[(zc) + 3];		\
} while (0)
...
#define SKS(S, o0, o1, o2, o3, d0, d1, d2, d3) do {	\
    ...
    r3 = w ## o3;					\
    S(r0, r1, r2, r3, r4);				\
    cs->sosemanuk.kc[i ++] = r ## d0;			\
    cs->sosemanuk.kc[i ++] = r ## d1;			\
    cs->sosemanuk.kc[i ++] = r ## d2;			\
    cs->sosemanuk.kc[i ++] = r ## d3;			\
} while (0)
...
#define SRD(S, x0, x1, x2, x3, ooff) do {			\
    S(u0, u1, u2, u3, u4);					\
    STORE32L(u ## x0 ^ v0, cs->sosemanuk.buf + ooff);		\
    STORE32L(u ## x1 ^ v1, cs->sosemanuk.buf + ooff +  4);	\
    STORE32L(u ## x2 ^ v2, cs->sosemanuk.buf + ooff +  8);	\
    STORE32L(u ## x3 ^ v3, cs->sosemanuk.buf + ooff + 12);	\
} while (0)
...
#define WUP(wi, wi5, wi3, wi1, cc) do {						\
    unsigned tt = (wi) ^ (wi5) ^ (wi3) ^ (wi1) ^ (0x9E3779B9 ^ (unsigned)(cc));	\
    (wi) = ROLc(tt, 11);							\
} while (0)
...
static inline void _xorbuf(const unsigned char *in1, const unsigned char *in2, unsigned char *out, unsigned long datalen)
{
    while (datalen -- > 0) {
	*out ++ = *in1 ++ ^ *in2 ++;
    }
...
static inline void _sosemanuk_internal(cipher_state *cs)
{
    unsigned s00 = cs->sosemanuk.s00;
    unsigned s01 = cs->sosemanuk.s01;
    unsigned s02 = cs->sosemanuk.s02;
    unsigned s03 = cs->sosemanuk.s03;
    unsigned s04 = cs->sosemanuk.s04;
    unsigned s05 = cs->sosemanuk.s05;
    unsigned s06 = cs->sosemanuk.s06;
    unsigned s07 = cs->sosemanuk.s07;
    unsigned s08 = cs->sosemanuk.s08;
    unsigned s09 = cs->sosemanuk.s09;
    unsigned r1 = cs->sosemanuk.r1;
    unsigned r2 = cs->sosemanuk.r2;
    ...
    cs->sosemanuk.s00 = s00;
    cs->sosemanuk.s01 = s01;
    cs->sosemanuk.s02 = s02;
    cs->sosemanuk.s03 = s03;
    cs->sosemanuk.s04 = s04;
    cs->sosemanuk.s05 = s05;
    cs->sosemanuk.s06 = s06;
    cs->sosemanuk.s07 = s07;
    cs->sosemanuk.s08 = s08;
    cs->sosemanuk.s09 = s09;
    cs->sosemanuk.r1 = r1;
    cs->sosemanuk.r2 = r2;
}

static inline int _sosemanuk_crypt(const unsigned char *in, unsigned char *out, unsigned long inlen, cipher_state *cs)
{
    if (cs->sosemanuk.ptr < (sizeof(cs->sosemanuk.buf))) {
	unsigned long rlen = (sizeof(cs->sosemanuk.buf)) - cs->sosemanuk.ptr;

	if (rlen > inlen) {
	    rlen = inlen;
	}
	_xorbuf(cs->sosemanuk.buf + cs->sosemanuk.ptr, in, out, rlen);
	in += rlen;
	out += rlen;
	inlen -= rlen;
	cs->sosemanuk.ptr += rlen;
    }
    while (inlen > 0) {
	_sosemanuk_internal(cs);
	if (inlen >= sizeof(cs->sosemanuk.buf)) {
	    _xorbuf(cs->sosemanuk.buf, in, out, sizeof(cs->sosemanuk.buf));
	    in += sizeof(cs->sosemanuk.buf);
	    out += sizeof(cs->sosemanuk.buf);
	    inlen -= sizeof(cs->sosemanuk.buf);
	} else {
	    _xorbuf(cs->sosemanuk.buf, in, out, inlen);
	    cs->sosemanuk.ptr = (int)inlen;
	    inlen = 0;
	}
    }
...
int sosemanuk_setiv(const unsigned char *iv, unsigned long ivlen, cipher_state *cs)
{
    ...
    if (ivlen > 0) memcpy(ivtmp, iv, ivlen);
    ...
    cs->sosemanuk.ptr = sizeof(cs->sosemanuk.buf);
...
int sosemanuk_setup(const unsigned char *key, unsigned long keylen, int num_rounds, cipher_state *cs)
{
    ...
    memcpy(wbuf, key, keylen);
    if (keylen < 32) {
	wbuf[keylen] = 0x01;
	if (keylen < 31) {
	    memset(wbuf + keylen + 1, 0, 31 - keylen);
	}
    }
