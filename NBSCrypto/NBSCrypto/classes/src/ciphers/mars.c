//
//	mars.c
//	Authors / Developers		: IBM, Don Coppersmith
//	Last Modified (Original)	: 2000 (1998)
//
...
static const unsigned int btab[4] =
{
    0xA4A8D57B, 0x5B5D193B, 0xC8A8309B, 0x73F9A978
};

static const unsigned int sbox[512] =
{
...
{
    int i, j, n;
    unsigned int m,p,r,t[15],t1,t2,w;
...
int mars_encrypt(const unsigned char *pt, unsigned char *ct, const cipher_state *cs)
{
    ...
    CORE(a, b, c, d, cs->mars.k[ 4], cs->mars.k[ 5]);
    CORE(b, c, d, a, cs->mars.k[ 6], cs->mars.k[ 7]);
    CORE(c, d, a, b, cs->mars.k[ 8], cs->mars.k[ 9]);
    CORE(d, a, b, c, cs->mars.k[10], cs->mars.k[11]);
    CORE(a, b, c, d, cs->mars.k[12], cs->mars.k[13]);
    CORE(b, c, d, a, cs->mars.k[14], cs->mars.k[15]);
    CORE(c, d, a, b, cs->mars.k[16], cs->mars.k[17]);
    CORE(d, a, b, c, cs->mars.k[18], cs->mars.k[19]);
    CORE(a, d, c, b, cs->mars.k[20], cs->mars.k[21]);
    CORE(b, a, d, c, cs->mars.k[22], cs->mars.k[23]);
    CORE(c, b, a, d, cs->mars.k[24], cs->mars.k[25]);
    CORE(d, c, b, a, cs->mars.k[26], cs->mars.k[27]);
    CORE(a, d, c, b, cs->mars.k[28], cs->mars.k[29]);
    CORE(b, a, d, c, cs->mars.k[30], cs->mars.k[31]);
    CORE(c, b, a, d, cs->mars.k[32], cs->mars.k[33]);
    CORE(d, c, b, a, cs->mars.k[34], cs->mars.k[35]);
...
int mars_decrypt(const unsigned char *ct, unsigned char *pt, const cipher_state *cs)
{
    ...
    CORE_INV(d, c, b, a, cs->mars.k[34], cs->mars.k[35]);
    CORE_INV(c, b, a, d, cs->mars.k[32], cs->mars.k[33]);
    CORE_INV(b, a, d, c, cs->mars.k[30], cs->mars.k[31]);
    CORE_INV(a, d, c, b, cs->mars.k[28], cs->mars.k[29]);
    CORE_INV(d, c, b, a, cs->mars.k[26], cs->mars.k[27]);
    CORE_INV(c, b, a, d, cs->mars.k[24], cs->mars.k[25]);
    CORE_INV(b, a, d, c, cs->mars.k[22], cs->mars.k[23]);
    CORE_INV(a, d, c, b, cs->mars.k[20], cs->mars.k[21]);
    CORE_INV(d, a, b, c, cs->mars.k[18], cs->mars.k[19]);
    CORE_INV(c, d, a, b, cs->mars.k[16], cs->mars.k[17]);
    CORE_INV(b, c, d, a, cs->mars.k[14], cs->mars.k[15]);
    CORE_INV(a, b, c, d, cs->mars.k[12], cs->mars.k[13]);
    CORE_INV(d, a, b, c, cs->mars.k[10], cs->mars.k[11]);
    CORE_INV(c, d, a, b, cs->mars.k[ 8], cs->mars.k[ 9]);
    CORE_INV(b, c, d, a, cs->mars.k[ 6], cs->mars.k[ 7]);
    CORE_INV(a, b, c, d, cs->mars.k[ 4], cs->mars.k[ 5]);
