//
//	sober128.c
//	Authors / Developers		: Philip Hawkes, Gregory G. Rose
...
const struct cipher_descriptor sober128_desc =
{
    "sober-128",
    37,
    16, 16, 16, 0,
...
#define ADDKEY(k) cs->sober128.R[KEYP] += (k);
#define DROUND(z) STEP(cs->sober128.R,z); NLFUNC(cs,(z+1)); cs->sober128.R[OFF((z+1),FOLDP)] ^= t;
#define SROUND(z) STEP(cs->sober128.R,z); NLFUNC(cs,(z+1)); _XORWORD(t, in+(z*4), out+(z*4));

#define NLFUNC(cs,z) {										\
    t = cs->sober128.R[OFF(z,0)] + cs->sober128.R[OFF(z,16)];					\
    t ^= Sbox[(t >> 24) & 0xFF];								\
    t = RORc(t, 8);										\
    t = ((t + cs->sober128.R[OFF(z,1)]) ^ cs->sober128.konst) + cs->sober128.R[OFF(z,6)];	\
    t ^= Sbox[(t >> 24) & 0xFF];								\
    t = t + cs->sober128.R[OFF(z,13)];								\
}

#define OFF(zero, i) (((zero)+(i)) % N)
#define RORc(x, y) ( ((((unsigned)(x)&0xFFFFFFFFUL)>>(unsigned)((y)&31)) | ((unsigned)(x)<<(unsigned)((32-((y)&31))&31))) & 0xFFFFFFFFUL)
#define STEP(R,z) R[OFF(z,0)] = R[OFF(z,15)] ^ R[OFF(z,4)] ^ (R[OFF(z,0)] << 8) ^ Multab[(R[OFF(z,0)] >> 24) & 0xFF];

#define XORNL(nl) \
    cs->sober128.R[FOLDP] ^= (nl);
...
static const unsigned Multab[256] = {
...
static const unsigned Sbox[256] = {
...
static inline unsigned _BYTE2WORD(const unsigned char *b)
{
    unsigned t;
    LOAD32L(t, b);
    return t;
...
static inline void _cycle(unsigned *R)
{
    int i;
    unsigned t;
...
static inline unsigned _nltap(const cipher_state *cs)
{
    unsigned t;
    NLFUNC(cs, 0);
    return t;
...
static inline void _XORWORD(unsigned w, const unsigned char *in, unsigned char *out)
{
    unsigned t;
    LOAD32L(t, in);
    t ^= w;
...
static inline void _sober128_diffuse(cipher_state *cs)
{
    unsigned t;
    DROUND(0);
    DROUND(1);
...
static inline void _sober128_genkonst(cipher_state *cs)
{
    unsigned newkonst;

    do {
	_cycle(cs->sober128.R);
	newkonst = _nltap(cs);
    } while ((newkonst & 0xFF000000) == 0);
    cs->sober128.konst = newkonst;
}

static inline void _sober128_reloadstate(cipher_state *cs)
{
    int i;

    for (i = 0; i < N; ++i) {
	cs->sober128.R[i] = cs->sober128.initR[i];
    }
}

static inline void _sober128_savestate(cipher_state *cs)
{
    int i;
    for (i = 0; i < N; ++i) {
	cs->sober128.initR[i] = cs->sober128.R[i];
    }
}

static inline int _sober128_crypt(const unsigned char *in, unsigned char *out, unsigned long inlen, cipher_state *cs)
{
    unsigned t;

    if (inlen == 0) return NBSCrypto_OK;

    while (cs->sober128.nbuf != 0 && inlen != 0) {
	*out++ = *in++ ^ (unsigned char)(cs->sober128.sbuf & 0xFF);
	cs->sober128.sbuf >>= 8;
	cs->sober128.nbuf -= 8;
	--inlen;
    }
...
int sober128_setup(const unsigned char *key, unsigned long keylen, int num_rounds, cipher_state *cs)
{
    ...
    cs->sober128.R[1] = 1;
    for (i = 2; i < N; ++i) {
	cs->sober128.R[i] = cs->sober128.R[i-1] + cs->sober128.R[i-2];
    }
    cs->sober128.konst = INITKONST;
    ...
    _sober128_diffuse(cs);
    _sober128_genkonst(cs);
    _sober128_savestate(cs);
    cs->sober128.nbuf = 0;
