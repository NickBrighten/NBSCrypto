//
//	seed.c
//	Authors / Developers		: KISA (Korea Internet & Security Agency)
...
#pragma mark - INLINE
static inline void _rounds(unsigned *P, const unsigned *K)
{
    unsigned T, T2;
    int     i;
    for (i = 0; i < 16; i += 2) {
...
int seed_setup(const unsigned char *key, int keylen, int num_rounds, cipher_state *cs)
{
    ...
    for (i = 0; i < 16; i++) {
	cs->seed.eK[2*i+0] = G(k1 + k3 - KCi[i]);
	cs->seed.eK[2*i+1] = G(k2 - k4 + KCi[i]);
	if (i&1) {
	    tmp = k3;
	...
	cs->seed.dK[2*(15-i)+0] = cs->seed.eK[2*i+0];
	cs->seed.dK[2*(15-i)+1] = cs->seed.eK[2*i+1];
    }
