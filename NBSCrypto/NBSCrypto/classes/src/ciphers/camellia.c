//
//	camellia.c
//	Authors / Developers		: Mitsubishi Electric, NTT
...
const struct cipher_descriptor camellia_desc =
{
    "camellia",
    6,
    16, 32, 16, 18,
...
static const unsigned long long key_sigma[] = {
    CONST64(0xA09E667F3BCC908B), CONST64(0xB67AE8584CAA73B2), CONST64(0xC6EF372FE94F82BE),
    CONST64(0x54FF53A5F1D36F1C), CONST64(0x10E527FADE682D1D), CONST64(0xB05688C2B3E6C1FD)
};
...
#pragma mark - INLINE
static inline unsigned long long _F(unsigned long long x)
{
    unsigned D, U;
...
static inline void _rot_128(const unsigned char *in, unsigned count, unsigned char *out)
{
    unsigned x, w, b;
...
int camellia_setup(const unsigned char *key, int keylen, int num_rounds, cipher_state *cs)
{
    ...
    cs->camellia.R = (keylen == 16) ? 18 : 24;

    if (num_rounds != 0 && num_rounds != cs->camellia.R) {
	return NBSCrypto_ERROR;
    }
    ...
    LOAD64(A, T+32); LOAD64(B, T+40);
    B ^= _F(A ^ key_sigma[0]);
    A ^= _F(B ^ key_sigma[1]);
    STORE64(A, T+32); STORE64(B, T+40);
    ...
    if (keylen == 16) {
	LOAD64(cs->camellia.kw[0], kL);
	LOAD64(cs->camellia.kw[1], kL+8);
	LOAD64(cs->camellia.k[0], kA);
	LOAD64(cs->camellia.k[1], kA+8);
	_rot_128(kL, 15, T+32);
	LOAD64(cs->camellia.k[2], T+32);
	LOAD64(cs->camellia.k[3], T+40);
	_rot_128(kA, 15, T+32);
	LOAD64(cs->camellia.k[4], T+32);
	LOAD64(cs->camellia.k[5], T+40);
	_rot_128(kA, 30, T+32);
	LOAD64(cs->camellia.kl[0], T+32);
	LOAD64(cs->camellia.kl[1], T+40);
	_rot_128(kL, 45, T+32);
	LOAD64(cs->camellia.k[6], T+32);
	LOAD64(cs->camellia.k[7], T+40);
	_rot_128(kA, 45, T+32);
	LOAD64(cs->camellia.k[8], T+32);
	_rot_128(kL, 60, T+32);
	LOAD64(cs->camellia.k[9], T+40);
	_rot_128(kA, 60, T+32);
	LOAD64(cs->camellia.k[10], T+32);
	LOAD64(cs->camellia.k[11], T+40);
	_rot_128(kL, 77, T+32);
	LOAD64(cs->camellia.kl[2], T+32);
	LOAD64(cs->camellia.kl[3], T+40);
	_rot_128(kL, 94, T+32);
	LOAD64(cs->camellia.k[12], T+32);
	LOAD64(cs->camellia.k[13], T+40);
	_rot_128(kA, 94, T+32);
	LOAD64(cs->camellia.k[14], T+32);
	LOAD64(cs->camellia.k[15], T+40);
	_rot_128(kL, 111, T+32);
	LOAD64(cs->camellia.k[16], T+32);
	LOAD64(cs->camellia.k[17], T+40);
	_rot_128(kA, 111, T+32);
	LOAD64(cs->camellia.kw[2], T+32);
	LOAD64(cs->camellia.kw[3], T+40);
    } else {
	LOAD64(A, T+32); LOAD64(B, T+40);
	B ^= _F(A ^ key_sigma[4]);
	A ^= _F(B ^ key_sigma[5]);
	...
	LOAD64(cs->camellia.kw[0], kL);
	LOAD64(cs->camellia.kw[1], kL+8);
	LOAD64(cs->camellia.k[0], kB);
	LOAD64(cs->camellia.k[1], kB+8);
	_rot_128(kR, 15, T+32);
	LOAD64(cs->camellia.k[2], T+32);
	LOAD64(cs->camellia.k[3], T+40);
	_rot_128(kA, 15, T+32);
	...
	LOAD64(cs->camellia.k[17], T+40);
	_rot_128(kA, 77, T+32);
	LOAD64(cs->camellia.kl[4], T+32);
	LOAD64(cs->camellia.kl[5], T+40);
	_rot_128(kR, 94, T+32);
	LOAD64(cs->camellia.k[18], T+32);
	LOAD64(cs->camellia.k[19], T+40);
	_rot_128(kA, 94, T+32);
	LOAD64(cs->camellia.k[20], T+32);
	LOAD64(cs->camellia.k[21], T+40);
	_rot_128(kL, 111, T+32);
	LOAD64(cs->camellia.k[22], T+32);
	LOAD64(cs->camellia.k[23], T+40);
	_rot_128(kB, 111, T+32);
	LOAD64(cs->camellia.kw[2], T+32);
	LOAD64(cs->camellia.kw[3], T+40);
    }
...
int camellia_encrypt(const unsigned char *pt, unsigned char *ct, const cipher_state *cs)
{
    ...
    LOAD64(L, pt+0); LOAD64(R, pt+8);
    L ^= cs->camellia.kw[0];
    R ^= cs->camellia.kw[1];

    R ^= _F(L ^ cs->camellia.k[0]);
    L ^= _F(R ^ cs->camellia.k[1]);
    R ^= _F(L ^ cs->camellia.k[2]);
    L ^= _F(R ^ cs->camellia.k[3]);
    R ^= _F(L ^ cs->camellia.k[4]);
    L ^= _F(R ^ cs->camellia.k[5]);

    a = (unsigned)(L >> 32);
    b = (unsigned)(L & 0xFFFFFFFFUL);
    b ^= ROL((a & (unsigned)(cs->camellia.kl[0] >> 32)), 1);
    a ^= b | (cs->camellia.kl[0] & 0xFFFFFFFFU);
    L = (((unsigned long long)a) << 32) | b;

    a = (unsigned)(R >> 32);
    b = (unsigned)(R & 0xFFFFFFFFUL);
    a ^= b | (cs->camellia.kl[1] & 0xFFFFFFFFU);
    b ^= ROL((a & (unsigned)(cs->camellia.kl[1] >> 32)), 1);
    R = (((unsigned long long)a) << 32) | b;

    R ^= _F(L ^ cs->camellia.k[6]);
    L ^= _F(R ^ cs->camellia.k[7]);
    R ^= _F(L ^ cs->camellia.k[8]);
    L ^= _F(R ^ cs->camellia.k[9]);
    R ^= _F(L ^ cs->camellia.k[10]);
    L ^= _F(R ^ cs->camellia.k[11]);

    a = (unsigned)(L >> 32);
    b = (unsigned)(L & 0xFFFFFFFFUL);
    b ^= ROL((a & (unsigned)(cs->camellia.kl[2] >> 32)), 1);
    a ^= b | (cs->camellia.kl[2] & 0xFFFFFFFFU);
    L = (((unsigned long long)a) << 32) | b;

    a = (unsigned)(R >> 32);
    b = (unsigned)(R & 0xFFFFFFFFUL);
    a ^= b | (cs->camellia.kl[3] & 0xFFFFFFFFU);
    b ^= ROL((a & (unsigned)(cs->camellia.kl[3] >> 32)), 1);
    R = (((unsigned long long)a) << 32) | b;

    R ^= _F(L ^ cs->camellia.k[12]);
    L ^= _F(R ^ cs->camellia.k[13]);
    R ^= _F(L ^ cs->camellia.k[14]);
    L ^= _F(R ^ cs->camellia.k[15]);
    R ^= _F(L ^ cs->camellia.k[16]);
    L ^= _F(R ^ cs->camellia.k[17]);

    if (cs->camellia.R == 24) {
	a = (unsigned)(L >> 32);
	b = (unsigned)(L & 0xFFFFFFFFUL);
	b ^= ROL((a & (unsigned)(cs->camellia.kl[4] >> 32)), 1);
	a ^= b | (cs->camellia.kl[4] & 0xFFFFFFFFU);
	L = (((unsigned long long)a) << 32) | b;

	a = (unsigned)(R >> 32);
	b = (unsigned)(R & 0xFFFFFFFFUL);
	a ^= b | (cs->camellia.kl[5] & 0xFFFFFFFFU);
	b ^= ROL((a & (unsigned)(cs->camellia.kl[5] >> 32)), 1);
	R = (((unsigned long long)a) << 32) | b;

	R ^= _F(L ^ cs->camellia.k[18]);
	L ^= _F(R ^ cs->camellia.k[19]);
	R ^= _F(L ^ cs->camellia.k[20]);
	L ^= _F(R ^ cs->camellia.k[21]);
	R ^= _F(L ^ cs->camellia.k[22]);
	L ^= _F(R ^ cs->camellia.k[23]);
    }

    L ^= cs->camellia.kw[3];
    R ^= cs->camellia.kw[2];

    STORE64(R, ct+0); STORE64(L, ct+8);
