...
static const unsigned long long T0[256] = {
...
static const unsigned long long T1[256] = {
...
static const unsigned long long T2[256] = {
...
static const unsigned long long T3[256] = {
...
static const unsigned long long T4[256] = {
...
static const unsigned long long T5[256] = {
...
static const unsigned long long T6[256] = {
...
static const unsigned long long T7[256] = {
...
static const unsigned long long c[R + 1] = {
...
#pragma mark - INLINE
static inline void _khazad_crypt(const unsigned char *in, unsigned char *out, const unsigned long long *roundKey) {
    int r;
    unsigned long long state;

    state =
    ((unsigned long long)in[0] << 56) ^
    ...
    out[0] = (unsigned char)(state >> 56);
    out[1] = (unsigned char)(state >> 48);
    out[2] = (unsigned char)(state >> 40);
    out[3] = (unsigned char)(state >> 32);
    out[4] = (unsigned char)(state >> 24);
    out[5] = (unsigned char)(state >> 16);
    out[6] = (unsigned char)(state >>  8);
    out[7] = (unsigned char)(state      );
}
...
int khazad_setup(const unsigned char *key, int keylen, int num_rounds, cipher_state *cs)
{
    ...
    K2 =
    ((unsigned long long)key[ 0] << 56) ^
    ((unsigned long long)key[ 1] << 48) ^
    ((unsigned long long)key[ 2] << 40) ^
    ((unsigned long long)key[ 3] << 32) ^
    ((unsigned long long)key[ 4] << 24) ^
    ((unsigned long long)key[ 5] << 16) ^
    ((unsigned long long)key[ 6] <<  8) ^
    ((unsigned long long)key[ 7]      );
    K1 =
    ((unsigned long long)key[ 8] << 56) ^
    ((unsigned long long)key[ 9] << 48) ^
    ((unsigned long long)key[10] << 40) ^
    ((unsigned long long)key[11] << 32) ^
    ((unsigned long long)key[12] << 24) ^
    ((unsigned long long)key[13] << 16) ^
    ((unsigned long long)key[14] <<  8) ^
    ((unsigned long long)key[15]      );
	...
	T7[(int)(K1      ) & 0xff] ^
	c[r] ^ K2;
	K2 = K1; K1 = cs->khazad.eK[r];
    }

    cs->khazad.dK[0] = cs->khazad.eK[R];
    for (r = 1; r < R; r++) {
	K1 = cs->khazad.eK[R - r];
	cs->khazad.dK[r] =
