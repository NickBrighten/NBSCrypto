//
//	swifftx.c
//	Authors / Developers		: Vadim Lyubashevsky, Daniele Micciancio, Chris Peikert,
//					  Alon Rosen
...
#define HAIFA_INPUT_BLOCK_SIZE (SWIFFTX_INPUT_BLOCK_SIZE - SWIFFTX_OUTPUT_BLOCK_SIZE - HAIFA_NUM_OF_BITS_SIZE - HAIFA_SALT_SIZE)
...
unsigned char saltValueChar[HAIFA_SALT_SIZE] = {114, 238, 247, 26, 192, 28, 170, 160};

const unsigned char HAIFA_IV_224[SWIFFTX_OUTPUT_BLOCK_SIZE] = {
...
const unsigned char HAIFA_IV_256[SWIFFTX_OUTPUT_BLOCK_SIZE] = {
...
const unsigned char HAIFA_IV_384[SWIFFTX_OUTPUT_BLOCK_SIZE] = {
...
const unsigned char HAIFA_IV_512[SWIFFTX_OUTPUT_BLOCK_SIZE] = {
...
static void AddToCurrInBase256(unsigned char value[HAIFA_NUM_OF_BITS_SIZE], unsigned short toAdd)
{
    ...
    short i;
    unsigned char currValueInBase256[8] = {0};
...
int swifftx_done(hash_state *state, unsigned char *hashval)
{
    ...
    if(!toAddFinalBlock){
	memset(currInputBlock + SWIFFTX_OUTPUT_BLOCK_SIZE + sizeOfLastInputBlock, 0, numOfZeroBytesInPadding);
	for (i = 0; i < HAIFA_NUM_OF_BITS_SIZE; ++i){
	    currInputBlock[SWIFFTX_OUTPUT_BLOCK_SIZE + sizeOfLastInputBlock + numOfZeroBytesInPadding + i] = messageLengthChar[i];
	}
	currInputBlock[SWIFFTX_OUTPUT_BLOCK_SIZE + sizeOfLastInputBlock + numOfZeroBytesInPadding + HAIFA_NUM_OF_BITS_SIZE] = digestSizeMSB;
	currInputBlock[SWIFFTX_OUTPUT_BLOCK_SIZE + sizeOfLastInputBlock + numOfZeroBytesInPadding + HAIFA_NUM_OF_BITS_SIZE + 1] = digestSizeLSB;
    }else{
	if((HAIFA_INPUT_BLOCK_SIZE - sizeOfLastInputBlock) > 0){
	    memset(currInputBlock + SWIFFTX_OUTPUT_BLOCK_SIZE + sizeOfLastInputBlock, 0, HAIFA_INPUT_BLOCK_SIZE - sizeOfLastInputBlock);
	}
    }
