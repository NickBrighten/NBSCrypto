//
//	shavite3.c
//	Authors / Developers		: Eli Biham, Orr Dunkelman
...
static const unsigned int Table0[256] = {
...
static const unsigned int Table1[256] = {
...
static const unsigned int Table2[256] = {
...
static const unsigned int Table3[256] = {
...
#pragma mark - INLINE
static inline int _roundAESkeyfirst(unsigned int input[4], unsigned int output[4], const unsigned int key [4])
{
    int i;
...
static inline int _messageExpansion256(unsigned int rk[ExpandedMessageSize256], unsigned int salt[8], unsigned int counter[2])
{
    int i,j,k;
	...
	for (i=16+32*k;i<32+32*k;){
	    ...
	    _roundAESkeyfirst(input, output, salt);
	    ...
	    _roundAESkeyfirst(input,output,salt+4);
...
static inline int _messageExpansion512(unsigned int rk[ExpandedMessageSize512], unsigned int salt[16], unsigned int counter[4])
{
    int i,j,k;
    unsigned int input[4],output[4];
	...
	for (i=32+64*k;i<64+64*k;){
	    ...
	    _roundAESkeyfirst(input,output,salt+8);
	    ...
	    _roundAESkeyfirst(input,output,salt+12);
...
static inline void _E256(unsigned int pt[8], unsigned int ct[8], unsigned int message[16], unsigned int salt[8], unsigned int counter[2])
{
    unsigned int state[8], i, j;
    unsigned int input[4], output[4], temp;
    unsigned int rk[ExpandedMessageSize256];
    ...
    _messageExpansion256(rk, salt, counter);

    for (i=0;i<ExternalRounds256;i++){
	for (j=0;j<4;j++){
	    input[j]=state[4+j];
	}
	_roundAESkeyfirst(input, output, rk+12*i);

	for (j=0;j<4;j++){
	    input[j]=output[j];
	}
	_roundAESkeyfirst(input, output, rk+12*i+4);

	for (j=0;j<4;j++){
	    input[j]=output[j];
	}
	_roundAESkeyfirst(input, output, rk+12*i+8);
...
static inline void _E512(unsigned int pt[16], unsigned int ct[16], unsigned int message[32], unsigned int salt[16], unsigned int counter[4])
{
    unsigned int state[16],i,j;
    unsigned int input[4],output[4],temp;
    unsigned int rk[ExpandedMessageSize512];
    ...
    _messageExpansion512(rk,salt,counter);
	...
	_roundAESkeyfirst(input,output,rk+32*i);
	for (j=0;j<4;j++){
	    input[j]=output[j];
	}
	_roundAESkeyfirst(input,output,rk+32*i+4);
	for (j=0;j<4;j++){
	    input[j]=output[j];
	}
	_roundAESkeyfirst(input,output,rk+32*i+8);
	for (j=0;j<4;j++){
	    input[j]=output[j];
	}
	_roundAESkeyfirst(input,output,rk+32*i+12);
	...
	_roundAESkeyfirst(input,output,rk+32*i+16);
	for(j=0;j<4;j++){
	    input[j]=output[j];
	}
	_roundAESkeyfirst(input,output,rk+32*i+20);
	for(j=0;j<4;j++){
	    input[j]=output[j];
	}
	_roundAESkeyfirst(input,output,rk+32*i+24);
	for(j=0;j<4;j++){
	    input[j]=output[j];
	}
	_roundAESkeyfirst(input,output,rk+32*i+28);
...
static inline void _compress256(const unsigned char *message_block, unsigned char *chaining_value, unsigned long long counter, const unsigned char salt[32])
{
    int i;

    unsigned int pt[8],ct[8];
    unsigned int msg_u32[16];
    unsigned int salt_u32[8];
    unsigned int cnt[2];
    ...
    cnt[1]=(unsigned int)(counter>>32);
    cnt[0]=(unsigned int)(counter & 0xFFFFFFFFULL);

    _E256(pt, ct, msg_u32, salt_u32, cnt);
...
static inline void _compress512(const unsigned char *message_block, unsigned char *chaining_value, unsigned long long counter, const unsigned char salt[64])
{
    int i;
    unsigned int pt[16],ct[16];
    unsigned int msg_u32[32];
    unsigned int salt_u32[16];
    unsigned int cnt[4];
    ...
    _E512(pt, ct, msg_u32, salt_u32, cnt);
...
static inline int _shavite3_init(hash_state *hs, int hashbitlen)
{
    if (hashbitlen>512){ return NBSCrypto_ERROR;}
    if (hashbitlen<1){ return NBSCrypto_ERROR;}

    memset(hs->shavite3.salt, 0, 64);

    hs->shavite3.bitcount = 0;
    hs->shavite3.DigestSize = hashbitlen;

    memset(hs->shavite3.buffer, 0, 128);
    memset(hs->shavite3.chaining_value, 0, 64);


    if (hashbitlen<257){
	_compress256(hs->shavite3.buffer, hs->shavite3.chaining_value, 0x0ULL, hs->shavite3.salt);
	U16TO8_LITTLE(hs->shavite3.buffer, hashbitlen);
	_compress256(hs->shavite3.buffer, hs->shavite3.chaining_value, 0x0ULL, hs->shavite3.salt);
	hs->shavite3.BlockSize = 512;
    }else{
	_compress512(hs->shavite3.buffer, hs->shavite3.chaining_value, 0x0UL, hs->shavite3.salt);
	U16TO8_LITTLE(hs->shavite3.buffer, hashbitlen);
	_compress512(hs->shavite3.buffer, hs->shavite3.chaining_value, 0x0ULL, hs->shavite3.salt);
	hs->shavite3.BlockSize=1024;
    }
...
int shavite3_224_init(hash_state *hs){return _shavite3_init(hs, 224);}
int shavite3_256_init(hash_state *hs){return _shavite3_init(hs, 256);}
int shavite3_384_init(hash_state *hs){return _shavite3_init(hs, 384);}
int shavite3_512_init(hash_state *hs){return _shavite3_init(hs, 512);}
...
int shavite3_process(hash_state *hs, const unsigned char *in, unsigned long inlen)
{
    ...
    int len = (int)inlen>>3;
    int BlockSizeB = (hs->shavite3.BlockSize/8);
    int bufcnt= (hs->shavite3.bitcount>>3)%BlockSizeB;
    unsigned long long local_bitcount;

    if (hs->shavite3.bitcount&7ULL){
	hs->shavite3.bitcount &= ~7ULL;
    }

    local_bitcount=hs->shavite3.bitcount;
    hs->shavite3.bitcount += inlen;

    if (inlen&7){
	hs->shavite3.partial_byte = in[inlen>>3];
    }

    if (bufcnt + len < BlockSizeB){
	memcpy(&hs->shavite3.buffer[bufcnt], p, len);
	return NBSCrypto_OK;
    }

    if (bufcnt>0){
	memcpy(&hs->shavite3.buffer[bufcnt], p, BlockSizeB-bufcnt);

	p += BlockSizeB-bufcnt;
	len -= BlockSizeB-bufcnt;
	local_bitcount+=8*(BlockSizeB-bufcnt);

	if (hs->shavite3.DigestSize<257){
	    _compress256(hs->shavite3.buffer, hs->shavite3.chaining_value, local_bitcount, hs->shavite3.salt);
	}else{
	    _compress512(hs->shavite3.buffer, hs->shavite3.chaining_value, local_bitcount, hs->shavite3.salt);
	}
    }
	...
	if (hs->shavite3.DigestSize<257){
	    _compress256(p, hs->shavite3.chaining_value, local_bitcount, hs->shavite3.salt);
	}else{
	    _compress512(p, hs->shavite3.chaining_value, local_bitcount, hs->shavite3.salt);
	}
    }

    if (len>0){
	memcpy(hs->shavite3.buffer, p, len);
    }
...
int shavite3_done(hash_state *hs, unsigned char *out)
{
    ...
    if (hs->shavite3.DigestSize < 257){
	memcpy(result, hs->shavite3.chaining_value, 32);
    }else{
	memcpy(result, hs->shavite3.chaining_value, 64);
    }

    memset(block, 0, BlockSizeB);
    memcpy(block, hs->shavite3.buffer, bufcnt);

    block[bufcnt] = (hs->shavite3.partial_byte&\
		     ~((0x80 >> (hs->shavite3.bitcount&7))-1)) | (0x80 >> (hs->shavite3.bitcount&7));

    if (hs->shavite3.DigestSize<257){
	if (bufcnt>=BlockSizeB-10){
	    _compress256(block, result, hs->shavite3.bitcount, hs->shavite3.salt);

	    memset(block, 0, BlockSizeB);
	    U64TO8_LITTLE(block+BlockSizeB-10, hs->shavite3.bitcount);
	    U16TO8_LITTLE(block+BlockSizeB-2, hs->shavite3.DigestSize);

	    _compress256(block, result, 0x0ULL,hs->shavite3.salt);
	}else{
	    U64TO8_LITTLE(block+BlockSizeB-10, hs->shavite3.bitcount);
	    U16TO8_LITTLE(block+BlockSizeB-2, hs->shavite3.DigestSize);
	    _compress256(block, result, hs->shavite3.bitcount, hs->shavite3.salt);
	...
	if (bufcnt>=BlockSizeB-18){
	    _compress512(block, result, hs->shavite3.bitcount, hs->shavite3.salt);

	    memset(block, 0, BlockSizeB);
	    U64TO8_LITTLE(block+BlockSizeB-18, hs->shavite3.bitcount);
	    U16TO8_LITTLE(block+BlockSizeB-2, hs->shavite3.DigestSize);

	    _compress512(block, result, 0x0UL, hs->shavite3.salt);
	}else{
	    U64TO8_LITTLE(block+BlockSizeB-18, hs->shavite3.bitcount);
    ...
    for (i=0;i<(hs->shavite3.DigestSize+7)/8;i++){
	out[i]=result[i];
    }

    if ((hs->shavite3.DigestSize)&7){
	out[(hs->shavite3.DigestSize+7)/8] &= \
	(0xFF<<(8-((hs->shavite3.DigestSize)%8)))&0xFF;
    }
