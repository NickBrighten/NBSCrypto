//
//	lane.c
//	Authors / Developers		: Sebastiaan Indesteege, Bart Preneel
//	Last Modified (Original)	: October 29, 2008
//
...
static const unsigned int k[768] =
{
...
typedef unsigned char aes_state[4][4];
typedef aes_state lane256_state[2];
typedef aes_state lane512_state[4];




#pragma mark - INLINE
static inline unsigned char _select_byte_32(unsigned int word, int idx)
{
    return (word >> (3-idx)*8) & 0xFF;
}

static inline unsigned char _select_byte_64(unsigned long word, int idx)
{
    return (word >> (7-idx)*8) & 0xFF;
}

static inline void _swap_bytes(unsigned char *a, unsigned char *b)
{
    unsigned char tmp;
    tmp = *a;
    *a = *b;
...
static inline unsigned char _mul2(unsigned char a)
{
    return a&0x80 ? (a<<1) ^ 0x1B : a<<1;
}

static inline unsigned char _mul3(unsigned char a)
{
    return _mul2(a)^a;
}

static inline void _addConst_256(int r, lane256_state a)
{
    int i, j, m;
	...
	for(i=0; i!=4; i++) {
	    for(j=0; j!=4; j++) {
		a[m][j][i] ^= _select_byte_32(k[8*r+4*m+i], j);
	    }
	}
    }
}

static inline void _addConst_512(int r, lane512_state a)
{
    int i, j, m;
	...
	for(i=0; i!=4; i++) {
	    for(j=0; j!=4; j++) {
		a[m][j][i] ^= _select_byte_32( k[16*r+4*m+i], j);
	    }
	}
    }
}

static inline void _addCount_256(int r, lane256_state a, unsigned long c)
{
    int j;
    for(j=0; j!=4; j++) {
	a[0][j][3] ^= _select_byte_64(c,4*(r%2)+j);
    }
}

static inline void _addCount_512(int r, lane512_state a, unsigned long  c)
{
    int j;
    for(j=0; j!=4; j++) {
	a[0][j][3] ^= _select_byte_64(c,4*(r%2)+j);
...
static inline void _mixColumns_256(lane256_state a)
{
    unsigned char b[4][4];
    int i, j, m;
	...
	for(j = 0; j != 4; j++)
	    for(i = 0; i != 4; i++)
		b[i][j] = _mul2(a[m][i][j]) ^ _mul3(a[m][(i + 1) % 4][j]) ^ a[m][(i + 2) % 4][j] ^ a[m][(i + 3) % 4][j];
	for(i = 0; i != 4; i++)
	    for(j = 0; j != 4; j++) a[m][i][j] = b[i][j];
    }
}

static inline void _mixColumns_512(lane512_state a)
{
    unsigned char b[4][4];
    int i, j, m;
...
static inline void _shiftRows_256(lane256_state a)
{
...
static inline void _shiftRows_512(lane512_state a)
{
    unsigned char tmp[4];
    int i, j, m;
...
static inline void _subBytes_256(lane256_state a)
{
    int i, j, m;
...
static inline void _subBytes_512(lane512_state a)
{
    int i, j, m;
...
static inline void _swapColumns_256(lane256_state a)
{
    int i;
    for (i=0; i!=4; i++) {
	_swap_bytes(&a[0][i][2],&a[1][i][0]);
	_swap_bytes(&a[0][i][3],&a[1][i][1]);
    }
}

static inline void _swapColumns_512(lane512_state a)
{
    int i;
    for (i=0; i!=4; i++) {
	_swap_bytes(&a[0][i][1],&a[1][i][0]);
	_swap_bytes(&a[0][i][2],&a[2][i][0]);
	_swap_bytes(&a[0][i][3],&a[3][i][0]);
	_swap_bytes(&a[1][i][2],&a[2][i][1]);
	_swap_bytes(&a[1][i][3],&a[3][i][1]);
	_swap_bytes(&a[2][i][3],&a[3][i][2]);
    }
}

static inline void _P256(int j, lane256_state a, unsigned long c)
{
    int i, r;
...
static inline void _P512(int j, lane512_state a, unsigned long c)
{
    int i, r;
...
static inline void _Q256(int j, lane256_state a, unsigned long c)
{
    int i, r;
...
static inline void _Q512(int j, lane512_state a, unsigned long c)
{
    int i, r;
...
static inline void _xor_256(lane256_state a, lane256_state b)
{
    int i, j, m;
    for(m=0; m!=2; m++)
	for (i=0; i!=4; i++)
...
static inline void _xor_512(lane512_state a, lane512_state b)
{
    int i, j, m;
    for(m=0; m!=4; m++)
	for (i=0; i!=4; i++)
...
static inline void _lane_expandMessage_256(unsigned char *hashval, const unsigned char buffer[64], lane256_state W0, lane256_state W1, lane256_state W2, lane256_state W3, lane256_state W4, lane256_state W5)
{
    int i, j;
...
static inline void _lane_expandMessage_512(unsigned char *hashval, const unsigned char buffer[128], lane512_state W0, lane512_state W1, lane512_state W2, lane512_state W3, lane512_state W4, lane512_state W5)
{
    int i, j;
...
static inline void _lane_storeHash_256(unsigned char *hv, lane256_state W0)
{
    int i, j, m;
    for(m=0; m!=2; m++)
	for (i=0; i!=4; i++)
	    for (j=0; j!=4; j++)
		hv[16*m+4*i+j] = W0[m][j][i];
}

static inline void _lane_storeHash_512(unsigned char *hv, lane512_state W0)
{
    int i, j, m;
    for(m=0; m!=4; m++)
	for (i=0; i!=4; i++)
...
static inline void _lane_transform_256(hash_state *hs, const unsigned char buffer[64], const unsigned long counter)
{
    lane256_state W0, W1, W2, W3, W4, W5;

    _lane_expandMessage_256(hs->lane.hash, buffer, W0, W1, W2, W3, W4, W5);
    ...
    _lane_storeHash_256(hs->lane.hash, W0);
}

static inline void _lane_transform_512(hash_state *hs, const unsigned char buffer[128], const unsigned long counter)
{
    lane512_state W0, W1, W2, W3, W4, W5;

    _lane_expandMessage_512(hs->lane.hash, buffer, W0, W1, W2, W3, W4, W5);
    ...
    _lane_storeHash_512(hs->lane.hash, W0);
}


static inline int _lane_init(hash_state *hs, int len)
{
    int i;

    if (len != 224 && len != 256 && len != 384 && len != 512) {
	return NBSCrypto_ERROR;
    }

    hs->lane.hashbitlen   = len;
    hs->lane.databitcount = 0;

    memset(hs->lane.hash, 0, 64);
    memset(hs->lane.buffer, 0, 128);
    hs->lane.buffer[0] = 0x02;
    for (i=0; i!=4; i++) {
	hs->lane.buffer[i+1] = _select_byte_32(hs->lane.hashbitlen, i);
    }
    if (len <= 256) {
	_lane_transform_256(hs,hs->lane.buffer,0);
    } else {
	_lane_transform_512(hs,hs->lane.buffer,0);
    }
...
int lane_224_init(hash_state *hs){return _lane_init(hs, 224);}
int lane_256_init(hash_state *hs){return _lane_init(hs, 256);}
int lane_384_init(hash_state *hs){return _lane_init(hs, 384);}
int lane_512_init(hash_state *hs){return _lane_init(hs, 512);}

int lane_process(hash_state *hs, const unsigned char *in, unsigned long inlen)
{
    unsigned long bytes = inlen >> 3;
    const int BLOCKSIZE = hs->lane.hashbitlen <= 256 ? 64 : 128;
    const unsigned long buffill = BLOCKSIZE == 64 ?
    (hs->lane.databitcount >> 3) & 0x3f : (hs->lane.databitcount >> 3) & 0x7f;

    if (hs->lane.databitcount & 0x7) {
	return NBSCrypto_ERROR;
    }

    if (buffill) {
	const unsigned long n = buffill + bytes > BLOCKSIZE ? BLOCKSIZE-buffill : bytes;
	memcpy(hs->lane.buffer + buffill, in, n);
	hs->lane.databitcount += n << 3;
	if (buffill + n == BLOCKSIZE) {
	    if (hs->lane.hashbitlen <= 256) {
		_lane_transform_256(hs, hs->lane.buffer, hs->lane.databitcount);
	    } else {
		_lane_transform_512(hs, hs->lane.buffer, hs->lane.databitcount);
	    }
	}
	in += n;
	bytes -= n;
	...
	hs->lane.databitcount += BLOCKSIZE << 3;
	if (hs->lane.hashbitlen <= 256) {
	    _lane_transform_256(hs, in, hs->lane.databitcount);
	} else {
	    _lane_transform_512(hs, in, hs->lane.databitcount);
	}
	in += BLOCKSIZE;
	bytes -= BLOCKSIZE;
    }
    ...
    if (bytes || (inlen & 0x7)) {
	memcpy(hs->lane.buffer, in, inlen & 0x7 ? bytes+1 : bytes);
	hs->lane.databitcount += (bytes << 3) + (inlen & 0x7);
    }
...
int lane_done(hash_state *hs, unsigned char *out)
{
    ...
    if (BLOCKSIZE == 64) {
	if (hs->lane.databitcount & 0x1ff) {
	    const unsigned long n = (((hs->lane.databitcount - 1) >> 3) + 1) & 0x3f;
	    if (n < BLOCKSIZE)
		memset(hs->lane.buffer + n, 0, BLOCKSIZE-n);

	    hs->lane.buffer[(hs->lane.databitcount >> 3) & 0x3f] &= ~(0xff >> (hs->lane.databitcount & 0x7));
	    _lane_transform_256(hs, hs->lane.buffer, hs->lane.databitcount);
	}
    } else {
	if (hs->lane.databitcount & 0x3ff) {
	    const unsigned long n = (((hs->lane.databitcount - 1) >> 3) + 1) & 0x7f;
	    if (n < BLOCKSIZE)
		memset(hs->lane.buffer + n, 0, BLOCKSIZE-n);

	    hs->lane.buffer[(hs->lane.databitcount >> 3) & 0x7f] &= ~(0xff >> (hs->lane.databitcount & 0x7));
	    _lane_transform_512(hs, hs->lane.buffer, hs->lane.databitcount);
	}
    }

    memset(hs->lane.buffer, 0, BLOCKSIZE);

    hs->lane.buffer[0] = 0x00;

    for (i=0; i!=8; i++) {
	hs->lane.buffer[i+1] = _select_byte_64(hs->lane.databitcount, i);
    }
    ...
    memcpy(out, hs->lane.hash, hs->lane.hashbitlen/8);
