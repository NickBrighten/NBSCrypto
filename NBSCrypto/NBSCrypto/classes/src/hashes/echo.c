//
//	echo.c
//	Authors / Developers		: Ryad Benadjila, Olivier Billet, Henri Gilbert, Gilles Macario-Rat, Thomas Peyrin,
//					  Matt Robshaw, Yannick Seurin
...
#define LOADs(i) do {			\
    WL##i = hs->echo.state[2*i];	\
    WH##i = hs->echo.state[2*i+1];	\
} while(0);
...
#define FEEDforward do {					\
	...
	WL11 ^= WL15 ^ WL7 ^ WL3 ;				\
	WH11 ^= WH15 ^ WH7 ^ WH3 ;				\
	hs->echo.state[0] = hs->echo.CV[0] ^ WL8 ;		\
	hs->echo.state[1] = hs->echo.CV[1] ^ WH8 ;		\
	hs->echo.state[2] = hs->echo.CV[2] ^ WL9 ;		\
	hs->echo.state[3] = hs->echo.CV[3] ^ WH9 ;		\
	hs->echo.state[4] = hs->echo.CV[4] ^ WL10;		\
	...
	hs->echo.state[3]   = hs->echo.CV[3]   ^ WH1 ^ WH9 ;	\
	hs->echo.state[4]   = hs->echo.CV[4]   ^ WL2 ^ WL10;	\
	hs->echo.state[5]   = hs->echo.CV[5]   ^ WH2 ^ WH10;	\
	hs->echo.state[6]   = hs->echo.CV[6]   ^ WL3 ^ WL11;	\
	hs->echo.state[7]   = hs->echo.CV[7]   ^ WH3 ^ WH11;	\
	hs->echo.state[8+0] = hs->echo.CV[8+0] ^ WL4 ^ WL12;	\
	hs->echo.state[8+1] = hs->echo.CV[8+1] ^ WH4 ^ WH12;	\
	hs->echo.state[8+2] = hs->echo.CV[8+2] ^ WL5 ^ WL13;	\
	hs->echo.state[8+3] = hs->echo.CV[8+3] ^ WH5 ^ WH13;	\
	hs->echo.state[8+4] = hs->echo.CV[8+4] ^ WL6 ^ WL14;	\
	hs->echo.state[8+5] = hs->echo.CV[8+5] ^ WH6 ^ WH14;	\
	hs->echo.state[8+6] = hs->echo.CV[8+6] ^ WL7 ^ WL15;	\
	hs->echo.state[8+7] = hs->echo.CV[8+7] ^ WH7 ^ WH15;	\
    }								\
} while(0);
...
#pragma mark - INLINE
static inline unsigned char _echo_endianess()
{
    unsigned int T;
    unsigned char T_o[8];
...
static inline void _echo_pushString(unsigned long long num, unsigned char* string, unsigned char type)
{
    unsigned int j;
...
static inline void _echo_compress(hash_state *hs)
{
    unsigned long long
    ...
    if(hs->echo.cv_blocks < 8){
	hs->echo.CV[0] = hs->echo.state[0];
	hs->echo.CV[1] = hs->echo.state[1];
	hs->echo.CV[2] = hs->echo.state[2];
	hs->echo.CV[3] = hs->echo.state[3];
	hs->echo.CV[4] = hs->echo.state[4];
	hs->echo.CV[5] = hs->echo.state[5];
	hs->echo.CV[6] = hs->echo.state[6];
	hs->echo.CV[7] = hs->echo.state[7];
	for(r=1;r<4;r++){
	    hs->echo.CV[0] ^= hs->echo.state[8*r+0];
	    hs->echo.CV[1] ^= hs->echo.state[8*r+1];
	    hs->echo.CV[2] ^= hs->echo.state[8*r+2];
	    hs->echo.CV[3] ^= hs->echo.state[8*r+3];
	    hs->echo.CV[4] ^= hs->echo.state[8*r+4];
	    hs->echo.CV[5] ^= hs->echo.state[8*r+5];
	    hs->echo.CV[6] ^= hs->echo.state[8*r+6];
	    hs->echo.CV[7] ^= hs->echo.state[8*r+7];
	}
    } else {
	hs->echo.CV[0]  = hs->echo.state[0]  ^ hs->echo.state[16];
	hs->echo.CV[1]  = hs->echo.state[1]  ^ hs->echo.state[17];
	hs->echo.CV[2]  = hs->echo.state[2]  ^ hs->echo.state[18];
	hs->echo.CV[3]  = hs->echo.state[3]  ^ hs->echo.state[19];
	hs->echo.CV[4]  = hs->echo.state[4]  ^ hs->echo.state[20];
	hs->echo.CV[5]  = hs->echo.state[5]  ^ hs->echo.state[21];
	hs->echo.CV[6]  = hs->echo.state[6]  ^ hs->echo.state[22];
	hs->echo.CV[7]  = hs->echo.state[7]  ^ hs->echo.state[23];
	hs->echo.CV[8]  = hs->echo.state[8]  ^ hs->echo.state[24];
	hs->echo.CV[9]  = hs->echo.state[9]  ^ hs->echo.state[25];
	hs->echo.CV[10] = hs->echo.state[10] ^ hs->echo.state[26];
	hs->echo.CV[11] = hs->echo.state[11] ^ hs->echo.state[27];
	hs->echo.CV[12] = hs->echo.state[12] ^ hs->echo.state[28];
	hs->echo.CV[13] = hs->echo.state[13] ^ hs->echo.state[29];
	hs->echo.CV[14] = hs->echo.state[14] ^ hs->echo.state[30];
	hs->echo.CV[15] = hs->echo.state[15] ^ hs->echo.state[31];
    }

    hs->echo.counter += hs->echo.inlen;
    ...
    LOADstate;
    r = 2*(3+(hs->echo.cv_blocks >> 2));

    if(endian == ET_BIG_ENDIAN){
	reverse_state(md->echo.state);
    }
...
static inline int _echo_init(hash_state *hs, int outlen)
{
    int i;

    if((outlen >= 0) && (outlen <= 512)){
	hs->echo.outlen = outlen;
    }
    ...
    for(i=0; i<hs->echo.cv_blocks; i++){
	_echo_pushString(outlen, (unsigned char*)(hs->echo.state+2*i), ULL);
	hs->echo.state[2*i+1] = 0;
    }

    for(i=hs->echo.cv_blocks; i<16; i++){
	hs->echo.state[2*i]   = 0;
	hs->echo.state[2*i+1] = 0;
    }
    ...
    memset(hs->echo.SALT, 0, 16);

    endian = _echo_endianess();
...
int echo_224_init(hash_state *hs){return _echo_init(hs, 224);}
int echo_256_init(hash_state *hs){return _echo_init(hs, 256);}
int echo_384_init(hash_state *hs){return _echo_init(hs, 384);}
int echo_512_init(hash_state *hs){return _echo_init(hs, 512);}
...
int echo_process(hash_state *hs, const unsigned char *in, unsigned long inlen)
{
    ...
    cv_length    = (hs->echo.cv_blocks << 4);
    message      = (unsigned char*)(hs->echo.state)+cv_length;
    data_size    = (ilen >> 3);
    message_size = (hs->echo.inlen >> 3);

    if(ilen == 0){
	return NBSCrypto_OK;
    }

    if(hs->echo.inlen % 8 != 0){
	return 4;
    }
	...
	if(data_size >= length){
	    memcpy(message + message_size, in, length);
	    hs->echo.inlen += (length << 3);
	    _echo_compress(hs);
	    message_size = 0;
	    in += length;
	    data_size -= length;
	    ...
	    memcpy(message + message_size, in, data_size);
...
int echo_done(hash_state *hs, unsigned char *out)
{
    ...
    block_length = 128*(16 - hs->echo.cv_blocks);
    filled_space = hs->echo.inlen;

    message  = (unsigned char*)(hs->echo.state);
    message += (hs->echo.cv_blocks << 4);
    ...
    message  = (unsigned char*)(hs->echo.state);
    message += (hs->echo.cv_blocks << 4);
    cnt = hs->echo.counter + hs->echo.inlen;
    for(i=filled_space; i<(block_length-144); i++){
	message[i/8] &= (0xff ^ (1 << (7-(i%8))));
    }
    message += (256-16*hs->echo.cv_blocks-18);
    _echo_pushString(hs->echo.outlen, message, USINT);
    _echo_pushString(cnt, message+2, ULL);
    _echo_pushString(0,   message+10, ULL);
    _echo_compress(hs);

    if(hs->echo.outlen%8 == 0){
	memcpy(out, hs->echo.state, (hs->echo.outlen)/8);
    }
    else{
	((unsigned char*)hs->echo.state)[((hs->echo.outlen)/8)] =
	((unsigned char*)hs->echo.state)[((hs->echo.outlen)/8)] & (0xff << (8-(hs->echo.outlen%8)));
	memcpy(out, hs->echo.state, ((hs->echo.outlen)/8) + 1);
    }
