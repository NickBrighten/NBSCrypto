...
#define SB0(x) WHIRLPOOL_TAB0[x]
#define SB1(x) WHIRLPOOL_TAB1[x]
#define SB2(x) WHIRLPOOL_TAB2[x]
#define SB3(x) WHIRLPOOL_TAB3[x]
#define SB4(x) WHIRLPOOL_TAB4[x]
#define SB5(x) WHIRLPOOL_TAB5[x]
#define SB6(x) WHIRLPOOL_TAB6[x]
#define SB7(x) WHIRLPOOL_TAB7[x]
...
static const unsigned long long WHIRLPOOL_TAB0[] = {
...
static const unsigned long long WHIRLPOOL_TAB1[] = {
...
static const unsigned long long WHIRLPOOL_TAB2[] = {
...
static const unsigned long long WHIRLPOOL_TAB3[] = {
...
static const unsigned long long WHIRLPOOL_TAB4[] = {
...
static const unsigned long long WHIRLPOOL_TAB5[] = {
...
static const unsigned long long WHIRLPOOL_TAB6[] = {
...
static const unsigned long long WHIRLPOOL_TAB7[] = {
...
static const unsigned long long WHIRLPOOL_CONT[] = {
    CONST64(0x1823c6e887b8014f), CONST64(0x36a6d2f5796f9152), CONST64(0x60bc9b8ea30c7b35), CONST64(0x1de0d7c22e4bfe57),
    CONST64(0x157737e59ff04ada), CONST64(0x58c9290ab1a06b85), CONST64(0xbd5d10f4cb3e0567), CONST64(0xe427418ba77d95d8),
    CONST64(0xfbee7c66dd17479e), CONST64(0xca2dbf07ad5a8333), CONST64(0x6302aa71c81949d9),
};
...
#pragma mark - INLINE
static inline int _whirlpool_compress(hash_state *hs, const unsigned char *buf)
{
    unsigned long long K[2][8], T[3][8];
    int x, y;

    for (x = 0; x < 8; x++) {
	K[0][x] = hs->whirlpool.s[x];
...
{
    zeromem(&hs->whirlpool, sizeof(hs->whirlpool));
    return NBSCrypto_OK;
}
...
int whirlpool_process(hash_state *hs, const unsigned char *in, unsigned long inlen)
{
    ...
    if (hs->whirlpool.clen > sizeof(hs->whirlpool.buf)) {
	return NBSCrypto_ERROR;
    }
    if ((hs->whirlpool.len + inlen * 8) < hs->whirlpool.len) {
	return NBSCrypto_ERROR;
    }
    while (inlen > 0) {
	if (hs->whirlpool.clen == 0 && inlen >= 64) {
	    if ((err = _whirlpool_compress(hs, in)) != NBSCrypto_OK) {
		return err;
	    }
	    hs->whirlpool.len += 64 * 8;
	    in		+= 64;
	    inlen	-= 64;
	} else {
	    n = MIN(inlen, (64 - hs->whirlpool.clen));

	    memcpy(hs->whirlpool.buf + hs->whirlpool.clen, in, (size_t)n);

	    hs->whirlpool.clen += n;
	    in += n;
	    inlen -= n;

	    if (hs->whirlpool.clen == 64) {
		if ((err = _whirlpool_compress(hs, hs->whirlpool.buf)) != NBSCrypto_OK) {
		    return err;
		}
		hs->whirlpool.len += 8*64;
		hs->whirlpool.clen = 0;
	    }
	}
    }
...
int whirlpool_done(hash_state *hs, unsigned char *out)
{
    ...
    hs->whirlpool.len += hs->whirlpool.clen * 8;
    hs->whirlpool.buf[hs->whirlpool.clen++] = (unsigned char)0x80;

    if (hs->whirlpool.clen > 32) {
	while (hs->whirlpool.clen < 64) {
	    hs->whirlpool.buf[hs->whirlpool.clen++] = (unsigned char)0;
	}
	_whirlpool_compress(hs, hs->whirlpool.buf);
	hs->whirlpool.clen = 0;
    }

    while (hs->whirlpool.clen < 56) {
	hs->whirlpool.buf[hs->whirlpool.clen++] = (unsigned char)0;
    }

    STORE64H(hs->whirlpool.len, hs->whirlpool.buf+56);
    _whirlpool_compress(hs, hs->whirlpool.buf);

    for (i = 0; i < 8; i++) {
	STORE64H(hs->whirlpool.s[i], out+(8*i));
    }
