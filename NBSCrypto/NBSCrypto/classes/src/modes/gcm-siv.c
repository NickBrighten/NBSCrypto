//
//	gcm-siv.c
//	Authors / Developers		: Shay Gueron, Yehuda Lindell, Adam Langley
//	Last Modified (Original)	: 2015
//

#include "nbs_crypto.h"




#pragma mark - DEFINES
#define BPD (sizeof(unsigned long long) * 8)
#define M(x) ( ((x&8)>>3) | ((x&4)>>1) | ((x&2)<<1) | ((x&1)<<3) )
#define WPV (1 + (16 / sizeof(unsigned long long)))

#define LOAD64H(x, y)					\
    do {x = (((unsigned long long)((y)[0] & 255))<<56)|	\
	    (((unsigned long long)((y)[1] & 255))<<48)|	\
	    (((unsigned long long)((y)[2] & 255))<<40)|	\
	    (((unsigned long long)((y)[3] & 255))<<32)|	\
	    (((unsigned long long)((y)[4] & 255))<<24)|	\
	    (((unsigned long long)((y)[5] & 255))<<16)|	\
	    (((unsigned long long)((y)[6] & 255))<< 8)|	\
	    (((unsigned long long)((y)[7] & 255)));	\
    } while(0)

#define LOAD64L(x, y)					\
    do {x = (((unsigned long long)((y)[7] & 255))<<56)|	\
	    (((unsigned long long)((y)[6] & 255))<<48)|	\
	    (((unsigned long long)((y)[5] & 255))<<40)|	\
	    (((unsigned long long)((y)[4] & 255))<<32)|	\
	    (((unsigned long long)((y)[3] & 255))<<24)|	\
	    (((unsigned long long)((y)[2] & 255))<<16)|	\
	    (((unsigned long long)((y)[1] & 255))<< 8)|	\
	    (((unsigned long long)((y)[0] & 255)));	\
    }while(0)

#define STORE32L(x, y)					\
    do {(y)[3] = (unsigned char)(((x)>>24)&255);	\
	(y)[2] = (unsigned char)(((x)>>16)&255);	\
	(y)[1] = (unsigned char)(((x)>> 8)&255);	\
	(y)[0] = (unsigned char)((x)&255);		\
    } while(0)

#define STORE64H(x, y)					\
    do {(y)[0] = (unsigned char)(((x)>>56)&255);	\
	(y)[1] = (unsigned char)(((x)>>48)&255);	\
	(y)[2] = (unsigned char)(((x)>>40)&255);	\
	(y)[3] = (unsigned char)(((x)>>32)&255);	\
	(y)[4] = (unsigned char)(((x)>>24)&255);	\
	(y)[5] = (unsigned char)(((x)>>16)&255);	\
	(y)[6] = (unsigned char)(((x)>> 8)&255);	\
	(y)[7] = (unsigned char)((x)&255);		\
    } while(0)

#define STORE64L(x, y)					\
    do {(y)[7] = (unsigned char)(((x)>>56)&255);	\
	(y)[6] = (unsigned char)(((x)>>48)&255);	\
	(y)[5] = (unsigned char)(((x)>>40)&255);	\
	(y)[4] = (unsigned char)(((x)>>32)&255);	\
	(y)[3] = (unsigned char)(((x)>>24)&255);	\
	(y)[2] = (unsigned char)(((x)>>16)&255);	\
	(y)[1] = (unsigned char)(((x)>> 8)&255);	\
	(y)[0] = (unsigned char)((x)&255);		\
    } while(0)

static const unsigned char _st[256*2] = {
    
};

typedef struct {
    unsigned char H[16];
    unsigned char S[16];
} polyval_state;




#pragma mark - INLINE
static inline int _ecb_encrypt_block(const unsigned char *pt, unsigned char *ct, cm_ECB *ecb)
{
    if (cipher_descriptor[ecb->cipher].accel_ecb_encrypt != NULL) {
	return cipher_descriptor[ecb->cipher].accel_ecb_encrypt(pt, ct, 1, &ecb->cs);
    }
    return cipher_descriptor[ecb->cipher].encrypt(pt, ct, &ecb->cs);
}


static inline void _mulx_ghash(unsigned char *b)
{
    unsigned int carry = b[15] & 1;
    int x;

    for (x = 15; x > 0; x--) {
	b[x] = (unsigned char)((b[x] >> 1) | ((b[x-1] & 1) << 7));
    }
    b[0] >>= 1;
    if (carry) b[0] ^= 0xe1;
}

static inline void _gcm_gf_mult(const unsigned char *a, const unsigned char *b, unsigned char *c)
{
    int i, j, k, u;
    unsigned long long B[16][WPV], tmp[32 / sizeof(unsigned long long)], pB[16 / sizeof(unsigned long long)], zz, z;
    unsigned char pTmp[32];

    zeromem(B[0], sizeof(B[0]));
    zeromem(B[M(1)], sizeof(B[M(1)]));

    for (i = 0; i < 2; i++) {
	LOAD64H(B[M(1)][i], a + (i<<3));
	LOAD64L(pB[i],      b + (i<<3));
    }

    B[M(2)][0] = B[M(1)][0] >> 1;
    B[M(4)][0] = B[M(1)][0] >> 2;
    B[M(8)][0] = B[M(1)][0] >> 3;

    for (i = 1; i < (int)WPV; i++) {
	B[M(2)][i] = (B[M(1)][i-1] << (BPD-1)) | (B[M(1)][i] >> 1);
	B[M(4)][i] = (B[M(1)][i-1] << (BPD-2)) | (B[M(1)][i] >> 2);
	B[M(8)][i] = (B[M(1)][i-1] << (BPD-3)) | (B[M(1)][i] >> 3);
    }

    for (i = 0; i < (int)WPV; i++) {
	B[M(3)][i]  = B[M(1)][i] ^ B[M(2)][i];
	B[M(5)][i]  = B[M(1)][i] ^ B[M(4)][i];
	B[M(6)][i]  = B[M(2)][i] ^ B[M(4)][i];
	B[M(9)][i]  = B[M(1)][i] ^ B[M(8)][i];
	B[M(10)][i] = B[M(2)][i] ^ B[M(8)][i];
	B[M(12)][i] = B[M(8)][i] ^ B[M(4)][i];

	B[M(7)][i]  = B[M(3)][i] ^ B[M(4)][i];
	B[M(11)][i] = B[M(3)][i] ^ B[M(8)][i];
	B[M(13)][i] = B[M(1)][i] ^ B[M(12)][i];
	B[M(14)][i] = B[M(6)][i] ^ B[M(8)][i];
	B[M(15)][i] = B[M(7)][i] ^ B[M(8)][i];
    }

    zeromem(tmp, sizeof(tmp));

    for (i = (BPD/4)-1; i >= 0; i--) {
	for (j = 0; j < (int)(WPV-1); j++) {
	    u = (pB[j] >> ((i^1)<<2)) & 15;
	    for (k = 0; k < (int)WPV; k++) {
		tmp[k+j] ^= B[u][k];
	    }
	}
	if (i != 0) {
	    for (z = j = 0; j < (int)(32 / sizeof(unsigned long long)); j++) {
		zz = tmp[j] << (BPD-4);
		tmp[j] = (tmp[j] >> 4) | z;
		z = zz;
	    }
	}
    }

    for (i = 0; i < 4; i++) {
	STORE64H(tmp[i], pTmp + (i<<3));
    }

    for (i = 31; i >= 16; i--) {
	pTmp[i-16] ^= _st[((unsigned)pTmp[i]<<1)];
	pTmp[i-15] ^= _st[((unsigned)pTmp[i]<<1)+1];
    }

    for (i = 0; i < 16; i++) {
	c[i] = pTmp[i];
    }
}

static inline void _polyval_init(polyval_state *st, const unsigned char *auth_key)
{
    int i;

    for (i = 0; i < 16; i++) st->H[i] = auth_key[15 - i];
    _mulx_ghash(st->H);
    zeromem(st->S, 16);
}

static inline void _polyval_block(polyval_state *st, const unsigned char *X)
{
    unsigned char T[16];
    int i;

    for (i = 0; i < 16; i++) st->S[i] ^= X[15 - i];
    _gcm_gf_mult(st->S, st->H, T);
    memcpy(st->S, T, 16);
}

static inline void _polyval_data(polyval_state *st, const unsigned char *data, unsigned long len)
{
    unsigned long full = len / 16;
    unsigned long rem  = len % 16;
    unsigned long i;
    unsigned char block[16];

    for (i = 0; i < full; i++) _polyval_block(st, data + i * 16);
    if (rem) {
	zeromem(block, 16);
	memcpy(block, data + full * 16, rem);
	_polyval_block(st, block);
    }
}

static inline void _polyval_done(polyval_state *st, unsigned char *out)
{
    int i;
    for (i = 0; i < 16; i++) out[i] = st->S[15 - i];
}


static inline int _gcm_siv_derive_keys(int cipher, const unsigned char *K, unsigned long keylen, const unsigned char *N, unsigned char *auth_key, unsigned char *enc_key, cm_ECB *enc_ecb)
{
    int err;
    unsigned char input[16], block[16];
    unsigned long ctr, num_blocks;
    cm_ECB ecb;

    if ((err = ecb_start(cipher, K, (int)keylen, 0, &ecb)) != NBSCrypto_OK) return err;

    memcpy(input + 4, N, 12);

    num_blocks = (keylen == 16) ? 4 : 6;
    for (ctr = 0; ctr < num_blocks; ctr++) {
	STORE32L((unsigned)ctr, input);
	if ((err = _ecb_encrypt_block(input, block, &ecb)) != NBSCrypto_OK) goto cleanup;
	if (ctr < 2) {
	    memcpy(auth_key + 8 * ctr, block, 8);
	} else {
	    memcpy(enc_key + 8 * (ctr - 2), block, 8);
	}
    }

    err = ecb_start(cipher, enc_key, (int)keylen, 0, enc_ecb);

cleanup:
    ecb_done(&ecb);

    zeromem(input, sizeof(input));
    zeromem(block, sizeof(block));

    return err;
}




#pragma mark - FUNCTIONS
int gcm_siv_crypt(int cipher, const unsigned char *key, unsigned long keylen, const unsigned char *nonce, unsigned long noncelen, const unsigned char *aad, unsigned long aadlen, unsigned char *in, unsigned long inlen, unsigned char *out, unsigned char *tag, unsigned long *taglen, int direction)
{
    int err, ecb_started = 0, ctr_started = 0;
    unsigned char auth_key[16];
    unsigned char enc_key[32];
    unsigned char S[16], IC[16], expected_tag[16];
    unsigned char lenblk[16];
    const unsigned char *plaintext;
    cm_ECB enc_ecb;
    cm_CTR ctr;
    polyval_state pv;
    unsigned long i;

    if ((err = is_cipher_valid(cipher)) != NBSCrypto_OK) return err;
    if (cipher_descriptor[cipher].block_length != 16) return NBSCrypto_ERROR;
    if (keylen != 16 &&
	keylen != 20 &&
	keylen != 24 &&
	keylen != 28 &&
	keylen != 32)	return NBSCrypto_ERROR;
    if (noncelen != 12)	return NBSCrypto_ERROR;
    if (*taglen < 16) {
	*taglen = 16;
	return NBSCrypto_ERROR;
    }

    if ((err = _gcm_siv_derive_keys(cipher, key, keylen, nonce, auth_key, enc_key, &enc_ecb)) != NBSCrypto_OK){
	goto cleanup;
    }
    ecb_started = 1;

    if (direction == 0) {
	memcpy(IC, tag, 16);
	IC[15] |= 0x80;
	if ((err = ctr_start(cipher, IC, enc_key, (int)keylen, 0, CTR_COUNTER_LITTLE_ENDIAN | 4, &ctr)) != NBSCrypto_OK){
	    goto cleanup;
	}
	ctr_started = 1;
	if (inlen) {
	    if ((err = ctr_decrypt(in, out, inlen, &ctr)) != NBSCrypto_OK) goto cleanup;
	}
	ctr_done(&ctr);
	ctr_started = 0;
	plaintext = out;

    }
    else {
	plaintext = in;
    }

    _polyval_init(&pv, auth_key);
    _polyval_data(&pv, aad, aadlen);
    _polyval_data(&pv, plaintext, inlen);
    STORE64L((unsigned long long)aadlen * 8, lenblk);
    STORE64L((unsigned long long)inlen  * 8, lenblk + 8);
    _polyval_block(&pv, lenblk);
    _polyval_done(&pv, S);

    for (i = 0; i < 12; i++) S[i] ^= nonce[i];
    S[15] &= 0x7f;

    if ((err = _ecb_encrypt_block(S, expected_tag, &enc_ecb)) != NBSCrypto_OK)  goto cleanup;

    if (direction == 0) {
	err = mem_neq(expected_tag, tag, 16);
	if (err != NBSCrypto_OK) {
	    if (inlen) zeromem(out, inlen);
	    goto cleanup;
	}
    }
    else {
	memcpy(IC, expected_tag, 16);
	IC[15] |= 0x80;
	if ((err = ctr_start(cipher, IC, enc_key, (int)keylen, 0, CTR_COUNTER_LITTLE_ENDIAN | 4, &ctr)) != NBSCrypto_OK) {
	    goto cleanup;
	}
	ctr_started = 1;
	if (inlen) {
	    if ((err = ctr_encrypt(in, out, inlen, &ctr)) != NBSCrypto_OK) goto cleanup;
	}
	memcpy(tag, expected_tag, 16);
	*taglen = 16;
    }

cleanup:
    if (ctr_started) ctr_done(&ctr);
    if (ecb_started) ecb_done(&enc_ecb);

    zeromem(auth_key, sizeof(auth_key));
    zeromem(enc_key, sizeof(enc_key));
    zeromem(S, sizeof(S));
    zeromem(IC, sizeof(IC));
    zeromem(expected_tag, sizeof(expected_tag));
    zeromem(lenblk, sizeof(lenblk));
    zeromem(&pv, sizeof(pv));

    return err;
}
