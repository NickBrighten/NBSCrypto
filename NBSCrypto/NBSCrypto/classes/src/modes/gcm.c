//
//	gcm.c
//

#include "nbs_crypto.h"


#define CONST64(n) n ## ULL
#define BPD (sizeof(unsigned long long) * 8)
#define M(x) ( ((x&8)>>3) | ((x&4)>>1) | ((x&2)<<1) | ((x&1)<<3) )
#define WPV (1 + (16 / sizeof(unsigned long long)))

#define LOAD64H(x, y)					\
    do {x = (((unsigned long long)((y)[0] & 255))<<56)|	\
	(((unsigned long long)((y)[1] & 255))<<48)|	\
	(((unsigned long long)((y)[2] & 255))<<40)|	\
	(((unsigned long long)((y)[3] & 255))<<32)|	\
	(((unsigned long long)((y)[4] & 255))<<24)|	\
	(((unsigned long long)((y)[5] & 255))<<16)|	\
	(((unsigned long long)((y)[6] & 255))<<8)|	\
	(((unsigned long long)((y)[7] & 255)));	\
} while(0)

#define LOAD64L(x, y)					\
    do {x = (((unsigned long long)((y)[7] & 255))<<56)|	\
	(((unsigned long long)((y)[6] & 255))<<48)|	\
	(((unsigned long long)((y)[5] & 255))<<40)|	\
	(((unsigned long long)((y)[4] & 255))<<32)|	\
	(((unsigned long long)((y)[3] & 255))<<24)|	\
	(((unsigned long long)((y)[2] & 255))<<16)|	\
	(((unsigned long long)((y)[1] & 255))<<8)|	\
	(((unsigned long long)((y)[0] & 255)));		\
}while(0)

#define STORE64H(x, y)					\
    do {(y)[0] = (unsigned char)(((x)>>56)&255);	\
	(y)[1] = (unsigned char)(((x)>>48)&255);	\
	(y)[2] = (unsigned char)(((x)>>40)&255);	\
	(y)[3] = (unsigned char)(((x)>>32)&255);	\
	(y)[4] = (unsigned char)(((x)>>24)&255);	\
	(y)[5] = (unsigned char)(((x)>>16)&255);	\
	(y)[6] = (unsigned char)(((x)>>8)&255);		\
	(y)[7] = (unsigned char)((x)&255);		\
} while(0)


static const unsigned char _st[256*2] = {
    
};

static inline void _gcm_gf_mult(const unsigned char *a, const unsigned char *b, unsigned char *c)
{
    int i, j, k, u;
    unsigned long long B[16][WPV], tmp[32 / sizeof(unsigned long long)], pB[16 / sizeof(unsigned long long)], zz, z;
    unsigned char pTmp[32];

    zeromem(B[0], sizeof(B[0]));
    zeromem(B[M(1)], sizeof(B[M(1)]));

    for (i = 0; i < 2; i++) {
	LOAD64H(B[M(1)][i], a + (i<<3));
	LOAD64L(pB[i],      b + (i<<3));
    }

    B[M(2)][0] = B[M(1)][0] >> 1;
    B[M(4)][0] = B[M(1)][0] >> 2;
    B[M(8)][0] = B[M(1)][0] >> 3;

    for (i = 1; i < (int)WPV; i++) {
	B[M(2)][i] = (B[M(1)][i-1] << (BPD-1)) | (B[M(1)][i] >> 1);
	B[M(4)][i] = (B[M(1)][i-1] << (BPD-2)) | (B[M(1)][i] >> 2);
	B[M(8)][i] = (B[M(1)][i-1] << (BPD-3)) | (B[M(1)][i] >> 3);
    }

    for (i = 0; i < (int)WPV; i++) {
	B[M(3)][i]  = B[M(1)][i] ^ B[M(2)][i];
	B[M(5)][i]  = B[M(1)][i] ^ B[M(4)][i];
	B[M(6)][i]  = B[M(2)][i] ^ B[M(4)][i];
	B[M(9)][i]  = B[M(1)][i] ^ B[M(8)][i];
	B[M(10)][i] = B[M(2)][i] ^ B[M(8)][i];
	B[M(12)][i] = B[M(8)][i] ^ B[M(4)][i];

	B[M(7)][i]  = B[M(3)][i] ^ B[M(4)][i];
	B[M(11)][i] = B[M(3)][i] ^ B[M(8)][i];
	B[M(13)][i] = B[M(1)][i] ^ B[M(12)][i];
	B[M(14)][i] = B[M(6)][i] ^ B[M(8)][i];
	B[M(15)][i] = B[M(7)][i] ^ B[M(8)][i];
    }

    zeromem(tmp, sizeof(tmp));

    for (i = (BPD/4)-1; i >= 0; i--) {
	for (j = 0; j < (int)(WPV-1); j++) {
	    u = (pB[j] >> ((i^1)<<2)) & 15;
	    for (k = 0; k < (int)WPV; k++) {
		tmp[k+j] ^= B[u][k];
	    }
	}
	if (i != 0) {
	    for (z = j = 0; j < (int)(32 / sizeof(unsigned long long)); j++) {
		zz = tmp[j] << (BPD-4);
		tmp[j] = (tmp[j] >> 4) | z;
		z = zz;
	    }
	}
    }

    for (i = 0; i < 4; i++) {
	STORE64H(tmp[i], pTmp + (i<<3));
    }

    for (i = 31; i >= 16; i--) {
	pTmp[i-16] ^= _st[((unsigned)pTmp[i]<<1)];
	pTmp[i-15] ^= _st[((unsigned)pTmp[i]<<1)+1];
    }

    for (i = 0; i < 16; i++) {
	c[i] = pTmp[i];
    }
}

static inline void _gcm_mult_h(const cm_GCM *gcm, unsigned char *I)
{
    unsigned char T[16];

    _gcm_gf_mult(gcm->H, I, T);

    memcpy(I, T, 16);
}

static inline int _gcm_process(unsigned char *pt, unsigned char *ct, unsigned long len, int direction, cm_GCM *gcm)
{
    unsigned char b;
    unsigned long x;
    int y, err;

    if (gcm->buflen > 16 || gcm->buflen < 0) {
	return NBSCrypto_ERROR;
    }

    if ((err = is_cipher_valid(gcm->cipher)) != NBSCrypto_OK) {
	return err;
    }

    if (gcm->pttotlen / 8 + (unsigned long long)gcm->buflen + (unsigned long long)len >= CONST64(0xFFFFFFFE0)) {
	return NBSCrypto_ERROR;
    }

    if (gcm->mode == 0) {
	if ((err = gcm_add_aad(NULL, 0, gcm)) != NBSCrypto_OK){
	    return err;
	}
    }

    if (gcm->mode == 1) {
	if (gcm->buflen) {
	    gcm->totlen += gcm->buflen * CONST64(8);
	    _gcm_mult_h(gcm, gcm->X);
	}

	for (y = 15; y >= 12; y--) {
	    if (++gcm->Y[y] & 255) { break; }
	}

	if ((err = cipher_descriptor[gcm->cipher].encrypt(gcm->Y, gcm->buf, &gcm->cs)) != NBSCrypto_OK) {
	    return err;
	}

	gcm->buflen = 0;
	gcm->mode   = 2;
    }

    if (gcm->mode != 2) {
	return NBSCrypto_ERROR;
    }

    for (x = 0; x < len; x++) {
	if (gcm->buflen == 16) {
	    gcm->pttotlen += 128;
	    _gcm_mult_h(gcm, gcm->X);

	    for (y = 15; y >= 12; y--) {
		if (++gcm->Y[y] & 255) { break; }
	    }

	    if ((err = cipher_descriptor[gcm->cipher].encrypt(gcm->Y, gcm->buf, &gcm->cs)) != NBSCrypto_OK) {
		return err;
	    }
	    gcm->buflen = 0;
	}

	if (direction == 1) {
	    //encrypt
	    b = ct[x] = pt[x] ^ gcm->buf[gcm->buflen];
	} else {
	    //decrypt
	    b = ct[x];
	    pt[x] = ct[x] ^ gcm->buf[gcm->buflen];
	}
	gcm->X[gcm->buflen++] ^= b;
    }

    return NBSCrypto_OK;
}

int gcm_add_iv(const unsigned char *iv, unsigned long ivlen, cm_GCM *gcm)
{
    unsigned long x, y;
    int err;

    if (gcm->mode != 0) {
	return NBSCrypto_ERROR;
    }

    if (gcm->buflen >= 16 || gcm->buflen < 0) {
	return NBSCrypto_ERROR;
    }

    if ((err = is_cipher_valid(gcm->cipher)) != NBSCrypto_OK) {
	return err;
    }

    if (ivlen + gcm->buflen > 12) {
	gcm->ivmode |= 1;
    }

    for (x = 0; x < ivlen; x++) {
	gcm->buf[gcm->buflen++] = *iv++;

	if (gcm->buflen == 16) {
	    for (y = 0; y < 16; y++) {
		gcm->X[y] ^= gcm->buf[y];
	    }
	    _gcm_mult_h(gcm, gcm->X);
	    gcm->buflen = 0;
	    gcm->totlen += 128;
	}
    }

    return NBSCrypto_OK;
}

int gcm_add_aad(const unsigned char *aad, unsigned long aadlen, cm_GCM *gcm)
{
    unsigned long x;
    int err;

    if (gcm->buflen > 16 || gcm->buflen < 0) {
	return NBSCrypto_ERROR;
    }

    if ((err = is_cipher_valid(gcm->cipher)) != NBSCrypto_OK) {
	return err;
    }

    if (gcm->mode == 0) {
	if (gcm->buflen == 0 && gcm->totlen == 0) return NBSCrypto_ERROR;
	if (gcm->ivmode || gcm->buflen != 12) {
	    for (x = 0; x < (unsigned long)gcm->buflen; x++) {
		gcm->X[x] ^= gcm->buf[x];
	    }

	    if (gcm->buflen) {
		gcm->totlen += gcm->buflen * CONST64(8);
		_gcm_mult_h(gcm, gcm->X);
	    }

	    zeromem(gcm->buf, 8);
	    STORE64H(gcm->totlen, gcm->buf+8);

	    for (x = 0; x < 16; x++) {
		gcm->X[x] ^= gcm->buf[x];
	    }
	    _gcm_mult_h(gcm, gcm->X);

	    memcpy(gcm->Y, gcm->X, 16);
	    zeromem(gcm->X, 16);
	} else {
	    memcpy(gcm->Y, gcm->X, 12);
	    gcm->Y[12] = 0;
	    gcm->Y[13] = 0;
	    gcm->Y[14] = 0;
	    gcm->Y[15] = 1;
	}
	memcpy(gcm->Y_0, gcm->Y, 16);
	zeromem(gcm->buf, 16);
	gcm->buflen = 0;
	gcm->totlen = 0;
	gcm->mode   = 1;
    }

    if (gcm->mode != 1 || gcm->buflen >= 16) {
	return NBSCrypto_ERROR;
    }

    for (x=0; x < aadlen; x++) {
	gcm->X[gcm->buflen++] ^= *aad++;

	if (gcm->buflen == 16) {
	    _gcm_mult_h(gcm, gcm->X);
	    gcm->buflen = 0;
	    gcm->totlen += 128;
	}
    }

    return NBSCrypto_OK;
}

int gcm_start(int cipher, const unsigned char *key, unsigned long keylen, int num_rounds, cm_GCM *gcm)
{
    unsigned char B[16];
    int err;

    if ((err = is_cipher_valid(cipher)) != NBSCrypto_OK) {
	return err;
    }

    if (cipher_descriptor[cipher].block_length != 16) {
	return NBSCrypto_ERROR;
    }

    if ((err = cipher_descriptor[cipher].setup(key, (int)keylen, num_rounds, &gcm->cs)) != NBSCrypto_OK) {
	return err;
    }

    zeromem(B, 16);

    if ((err = cipher_descriptor[cipher].encrypt(B, gcm->H, &gcm->cs)) != NBSCrypto_OK) {
	return err;
    }

    zeromem(gcm->buf, sizeof(gcm->buf));
    zeromem(gcm->X,   sizeof(gcm->X));
    gcm->cipher   = cipher;
    gcm->mode     = 0;
    gcm->ivmode   = 0;
    gcm->buflen   = 0;
    gcm->totlen   = 0;
    gcm->pttotlen = 0;

    return NBSCrypto_OK;
}

int gcm_encrypt(unsigned char *pt, unsigned char *ct, unsigned long len, cm_GCM *gcm)
{
    return _gcm_process(pt, ct, len, 1, gcm);
}

int gcm_decrypt(unsigned char *ct, unsigned char *pt, unsigned long len, cm_GCM *gcm)
{
    return _gcm_process(pt, ct, len, 0, gcm);
}

int gcm_done(unsigned char *tag, unsigned long *taglen, cm_GCM *gcm)
{
    unsigned long x;
    int err;

    if (gcm->buflen > 16 || gcm->buflen < 0) {
	return NBSCrypto_ERROR;
    }

    if ((err = is_cipher_valid(gcm->cipher)) != NBSCrypto_OK) {
	return err;
    }

    if (gcm->mode == 0) {
	if ((err = gcm_add_aad(NULL, 0, gcm)) != NBSCrypto_OK){
	    return err;
	}
    }

    if (gcm->mode == 1) {
	if ((err = _gcm_process(NULL, NULL, 0, 0, gcm)) != NBSCrypto_OK){
	    return err;
	}
    }

    if (gcm->mode != 2) {
	return NBSCrypto_ERROR;
    }

    if (gcm->buflen) {
	gcm->pttotlen += gcm->buflen * CONST64(8);
	_gcm_mult_h(gcm, gcm->X);
    }

    STORE64H(gcm->totlen, gcm->buf);
    STORE64H(gcm->pttotlen, gcm->buf+8);

    for (x = 0; x < 16; x++) {
	gcm->X[x] ^= gcm->buf[x];
    }

    _gcm_mult_h(gcm, gcm->X);

    if ((err = cipher_descriptor[gcm->cipher].encrypt(gcm->Y_0, gcm->buf, &gcm->cs)) != NBSCrypto_OK) {
	return err;
    }

    for (x = 0; x < 16 && x < *taglen; x++) {
	tag[x] = gcm->buf[x] ^ gcm->X[x];
    }

    *taglen = x;

    cipher_descriptor[gcm->cipher].done(&gcm->cs);

    return NBSCrypto_OK;
}
