...
static inline void _four_rounds(pelican_state *pelmac)
{
    ...
    LOAD32(s0, pelmac->state      );
    LOAD32(s1, pelmac->state  +  4);
    LOAD32(s2, pelmac->state  +  8);
    LOAD32(s3, pelmac->state  + 12);
    for(r = 0; r < 4; r++){
	t0 = Te0(NBS_BYTE(s0, 3)) ^ Te1(NBS_BYTE(s1, 2)) ^ Te2(NBS_BYTE(s2, 1)) ^ Te3(NBS_BYTE(s3, 0));
	t1 = Te0(NBS_BYTE(s1, 3)) ^ Te1(NBS_BYTE(s2, 2)) ^ Te2(NBS_BYTE(s3, 1)) ^ Te3(NBS_BYTE(s0, 0));
	t2 = Te0(NBS_BYTE(s2, 3)) ^ Te1(NBS_BYTE(s3, 2)) ^ Te2(NBS_BYTE(s0, 1)) ^ Te3(NBS_BYTE(s1, 0));
	t3 = Te0(NBS_BYTE(s3, 3)) ^ Te1(NBS_BYTE(s0, 2)) ^ Te2(NBS_BYTE(s1, 1)) ^ Te3(NBS_BYTE(s2, 0));
	s0 = t0; s1 = t1; s2 = t2; s3 = t3;
    }
    STORE32(s0, pelmac->state      );
    STORE32(s1, pelmac->state  +  4);
    STORE32(s2, pelmac->state  +  8);
    STORE32(s3, pelmac->state  + 12);
}
...
int pelican_init(const unsigned char *key, unsigned long keylen, pelican_state *pelmac)
{
    ...
    if((err = aes_setup(key, (int)keylen, 0, &pelmac->cs)) != NBSCrypto_OK){
	return err;
    }

    zeromem(pelmac->state, 16);
    aes_encrypt(pelmac->state, pelmac->state, &pelmac->cs);
    pelmac->buflen = 0;
...
int pelican_done(unsigned char *out, pelican_state *pelmac)
{
    ...
    aes_encrypt(pelmac->state, out, &pelmac->cs);
    aes_done(&pelmac->cs);
